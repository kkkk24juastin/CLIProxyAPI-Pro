...
认证文件页的非运行时账号卡片还提供“测试连接”入口。弹窗先读取该认证文件注册的文本模型，再通过 `POST /v0/management/auth-files/test` 将一次最小真实生成请求固定到当前 `auth_index`；成功时展示模型输出和耗时，失败时展示上游 HTTP 状态、错误码与脱敏后的错误详情。被禁用或处于冷却状态的账号也可独立测试，测试不会改动用户设置的禁用开关。

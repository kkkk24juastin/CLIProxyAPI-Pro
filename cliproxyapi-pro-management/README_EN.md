...
`proxy-pool` registers its own page through the upstream plugin resource protocol. `pro-observability` registers an observability resource showing the sole plugin store, automatic migration, usage contract, backups, quota cache, and routing runtime. A controlled iframe message bridge reuses the active API base and management key, including remote-Core connections. The monitoring page keeps the `/usage*` contract while the plugin owns its data; Core retains only the SSE transport bridge.

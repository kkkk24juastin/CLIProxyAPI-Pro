...
} from "../src/services/api/oauthModelPolicy";

describe("oauth model policy service", () => {
  test("normalizes known plans and preserves fallback distinction", () => {
  ...
  test("serializes only explicitly configured rules", () => {
    const config = normalizeOAuthModelPolicyConfig({});
  ...
  test("validates positive Go duration fields", () => {
  ...
  test("converts duration values for fixed-unit controls", () => {
  ...
  test("normalizes every provider and preserves custom plan keys", () => {
    const config = normalizeOAuthModelPolicyConfig({
      providers: {

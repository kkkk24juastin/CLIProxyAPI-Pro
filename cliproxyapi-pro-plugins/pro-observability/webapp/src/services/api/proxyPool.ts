import { requestManagement } from '@/services/bridge';
...
const managementGet = <T>(path: string): Promise<T> =>
  requestManagement<T>('GET', path);
const managementPost = <T>(path: string, body?: unknown): Promise<T> =>
  requestManagement<T>('POST', path, { body });
const managementPut = <T>(path: string, body?: unknown): Promise<T> =>
  requestManagement<T>('PUT', path, { body });
const managementPatch = <T>(path: string, body?: unknown): Promise<T> =>
  requestManagement<T>('PATCH', path, { body });

const readGlobalProxyUrl = async (): Promise<string> => {
  const response = asRecord(await managementGet('/proxy-url'));
...
const waitForStatus = async (
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const status = normalizeStatus(await managementGet('/pro/proxy-pool/status'));
      if (
        status.ready &&
...
const loadBypassCredentials = async (): Promise<ProxyPoolBypassCredential[]> => {
  ...
  try {
    const response = asRecord(await managementGet('/auth-files'));
    ...
    // Advisory inventory failures must not block pool control.
  }
  try {
    const rawConfig = await managementGet<unknown>('/config');
    ...
    // Config inventory is advisory too.
  }
...
export const proxyPoolApi = {
  async load(): Promise<ProxyPoolSnapshot> {
    const [rawConfig, globalProxyUrl, bypassCredentials] = await Promise.all([
      managementGet<unknown>(`/plugins/${PROXY_POOL_PLUGIN_ID}/config`),
      readGlobalProxyUrl(),
      loadBypassCredentials(),
    ]);
    let status: ProxyPoolStatus | null = null;
    try {
      status = normalizeStatus(await managementGet('/pro/proxy-pool/status'));
    } catch {
      status = null;
    }
    ...
    return {
      ...
      pluginEnabled: true,
      pluginRegistered: Boolean(status?.ready),
      config,
      status,
      globalProxyUrl,
  ...
  async save(config: ProxyPoolConfig, preserveTakeover = false): Promise<ProxyPoolStatus> {
    ...
    await managementPatch(
      `/plugins/${PROXY_POOL_PLUGIN_ID}/config`,
      serializeProxyPoolConfig(config)
    );
    const status = await waitForStatus(config.listen, minimumGeneration);
    if (preserveTakeover) {
      await managementPut('/proxy-url', { value: status.proxyUrl });
    }
    return status;
  },
  ...
  async deactivate(config: ProxyPoolConfig): Promise<void> {
    await managementPut('/proxy-url', { value: config.restoreProxyUrl.trim() });
  },

  async status(): Promise<ProxyPoolStatus> {
    return normalizeStatus(await managementGet('/pro/proxy-pool/status'));
  },
  ...
  async testNode(
    ...
    try {
      return normalizeProbeResult(
        await managementPost('/pro/proxy-pool/test', {
          node_id: nodeId,
  ...
  async testAll(): Promise<ProxyPoolProbeResult[]> {
    const response = asRecord(
      await managementPost('/pro/proxy-pool/test-all', { concurrency: 4 })
    );
    return Array.isArray(response.results) ? response.results.map(normalizeProbeResult) : [];
  },

  resetStats: () => managementPost('/pro/proxy-pool/reset-stats'),

  recoverNode: (nodeId: string) =>
    managementPost('/pro/proxy-pool/recover', { node_id: nodeId }),
};

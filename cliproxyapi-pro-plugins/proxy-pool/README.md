...
The plugin registers its management page through `ManagementAPI` as the `/ui` resource; Management does not ship a second proxy-pool implementation. The page is built from the shared `pro-observability/webapp` source into this plugin's `web/index.html` and preserves the original responsive React UX, including batch paste (`URL` or `label | URL | weight`), draft testing, node sheets, takeover confirmation, manual recovery, and runtime diagnostics. Runtime health and counters are intentionally ephemeral; plugin configuration is persisted with the normal Core configuration.

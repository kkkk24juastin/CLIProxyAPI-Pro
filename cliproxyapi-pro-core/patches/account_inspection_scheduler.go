...
func newAccountInspectionScheduler(h *Handler) *accountInspectionScheduler {
	scheduler := &accountInspectionScheduler{
		...
		status: accountInspectionStatus{State: "idle", Progress: accountInspectionProgress{Status: "idle"}},
	}
...
func (s *accountInspectionScheduler) broadcastLocked(message accountInspectionLogStreamMessage) {
	...
	if message.Status.State == "" {
		message.Status = s.statusSnapshotLocked()
	}
	if message.Schedule.IntervalMinutes == 0 {
		message.Schedule = s.schedule
	}
...
func (s *accountInspectionScheduler) subscribeLogs() (chan accountInspectionLogStreamMessage, accountInspectionLogStreamMessage) {
	...
	return subscriber, accountInspectionLogStreamMessage{Type: "snapshot", Schedule: s.schedule, Status: status, Logs: status.Logs}
}
...
func (s *accountInspectionScheduler) startRun(manual bool) error {
	...
	s.status.LastError = ""
	s.status.Progress = accountInspectionProgress{Status: "running"}
...
func (s *accountInspectionScheduler) appendLog(level string, message string) {
	...
	s.broadcastLocked(accountInspectionLogStreamMessage{Type: "log", Status: s.statusSnapshotLocked(), Log: &entry})
}

func (s *accountInspectionScheduler) updateProgress(total int, completed int, inFlight int, state string) {
	if state == "" {
		state = "running"
	}
	...
	s.mu.Lock()
	s.status.Progress = accountInspectionProgress{Total: total, Completed: completed, InFlight: inFlight, Pending: pending, Status: state}
	s.broadcastLocked(accountInspectionLogStreamMessage{Type: "status", Status: s.statusSnapshotLocked()})
	s.mu.Unlock()
}
...
func (s *accountInspectionScheduler) waitIfPaused(ctx context.Context) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	for s.status.Paused && !s.status.Stopping {
...
func (s *accountInspectionScheduler) pauseRun() {
	...
	if s.status.Running && !s.status.Stopping {
		...
		s.status.Progress.Status = "paused"
		s.broadcastLocked(accountInspectionLogStreamMessage{Type: "status", Status: s.statusSnapshotLocked()})
	}
	s.mu.Unlock()
}
...
func (s *accountInspectionScheduler) resumeRun() {
	...
	if s.status.Running && s.status.Paused {
		s.status.Paused = false
		s.status.State = "running"
		s.status.Progress.Status = "running"
		s.broadcastLocked(accountInspectionLogStreamMessage{Type: "status", Status: s.statusSnapshotLocked()})
		s.pause.Broadcast()
	}
	s.mu.Unlock()
...
func (s *accountInspectionScheduler) stopRun() {
	...
	if s.status.Running {
		...
		s.status.Progress.Status = "stopping"
		s.broadcastLocked(accountInspectionLogStreamMessage{Type: "status", Status: s.statusSnapshotLocked()})
		s.pause.Broadcast()
	}
	s.mu.Unlock()
...
func (s *accountInspectionScheduler) run(ctx context.Context, cancel context.CancelFunc, schedule accountInspectionSchedule, manual bool) {
	...
	s.status.Progress.Status = state
...
func (s *accountInspectionScheduler) executeInspection(ctx context.Context, settings accountInspectionSettings) ([]accountInspectionResult, accountInspectionSummary, error) {
	...
	s.updateProgress(len(accounts), 0, 0, "running")
	for i := 0; i < workers && i < len(accounts); i++ {
		...
		go func() {
			...
			for {
				...
				progressMu.Lock()
				inFlight++
				s.updateProgress(len(accounts), completed, inFlight, "running")
				progressMu.Unlock()
...
func dedupeExecutionItems(items []accountInspectionResult) []accountInspectionResult {
	...
	for _, item := range items {
		if item.Action == "keep" || item.Action == "" || item.FileName == "" {
			continue
		}
		key := item.FileName
		if _, ok := seen[key]; ok {
